...
//upsync类型相关的配置，类型包括etcd/consul/zookeeper
typedef struct {
    ngx_str_t                                name;
...
//ngx_http_upsync_server_t包含了ngx_http_upsync_srv_conf_t，存放一个upstream的相关
...
typedef struct {
	...
	//ngx_http_upsync_srv_conf_t中包含ngx_http_upstream_server_t的类型
...
//ngx_http_upsync_server_t包含了ngx_http_upsync_srv_conf_t，ngx_http_upsync_main_conf_t包含ngx_http_upsync_server_t
typedef struct {
...
//ngx_http_upsync_module_ctx存放解析配置文件并初始化模块相关数据结构的回掉函数
//ngx_http_upsync_commands存放模块的配置指令和转换指令的函数
ngx_module_t  ngx_http_upsync_module = {
    NGX_MODULE_V1,
	//ngx_http_upsync_module_ctx和ngx_http_upsync_commands里的回调函数在nginx启动时调用init_module前即会调用？
...
static ngx_upsync_conf_t  ngx_upsync_types[] = {
	...
	//
	//TODO:添加关于zookeeper的ngx_upsync_conf_t数据结构
	//
    { ngx_null_string,
      0,
      NULL,
...
//指令upsync的回调函数,填充upsync_srv_conf_t类型的各个成员
static char *
ngx_http_upsync_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
	...
	//利用nginx提供的调用获取upsync_srv_conf_t变量的地址，然后填充内容
	//调用包括ngx_http_conf_get_module_main/srv/loc_conf()
    upscf = ngx_http_conf_get_module_srv_conf(cf,
                                              ngx_http_upsync_module);
	//还要根据解析初始化ngx_http_upsync_srv_conf_t的成员ngx_http_upstream_server_t
    conf_server = &upscf->conf_server;

    for (i = 2; i < cf->args->nelts; i++) {

		//value[0到nelts-1]是以空白符分割的配置参数
	...
	//解析数据后台的IP和PORT，TODO:由于zookeeper的特殊地址格式也许需要针对其修改代码
...
//指令upsync_dump_path的回掉函数，存储所获取配置的文件路径
static char *
...
ngx_http_upsync_process(ngx_http_upsync_server_t *upsync_server)
{
	...
	//检查consul的查询的index是否发生变化，只有检查cousul才可能返回NGX_ERROR
...
ngx_http_upsync_check_index(ngx_http_upsync_server_t *upsync_server)
{
	...
	//找到consul header的index是否发生变化,如果没有变化则出错
    ...
    ngx_http_upsync_server_t *upsync_server)
{
	...
	//此处ctx->upstream_conf为ngx_array_t
    len = ctx->upstream_conf.nelts;
	//遍历获取到的代理，看是否在已生效代理中，如不在，则加入
    for (i = 0; i < len; i++) {
...
//TODO:利用zookeeper返回结果，填充ngx_http_upsync_conf_t中的变量
...
ngx_http_upsync_init_main_conf(ngx_conf_t *cf, void *conf)
{
	...
	//初始化和填充ngx_http_upsync_main_conf_t后，赋值全局变量upsync_ctx，其中有个成员upsync_server
    upsync_ctx = upmcf;
    uscfp = umcf->upstreams.elts;

	//ngx_http_upstream_srv_conf_t的变量中有upstream的个数,循环中init_srv_conf
...
//初始化和填充ngx_http_upsync_srv_conf_t的变量
static void *
...
ngx_http_upsync_init_srv_conf(ngx_conf_t *cf, void *conf, ngx_uint_t num)
{
	...
	//upstream_srv_conf_t中可以找到upsync_srv_conf_t的配置数据
...
ngx_http_upsync_init_module(ngx_cycle_t *cycle)
{
	...
	//置为NGX_INVALID_FILE(-1)，修改存储upstream配置文件的读写属性
...
ngx_http_upsync_init_process(ngx_cycle_t *cycle)
{
	...
	//循环中从数据后台(consul etcd zookeeper)获取upstream代理后台
    for (i = 0; i < upsync_ctx->upstream_num; i++) {
	//upsync_type_conf存放和具体后台(consul etcd zookeeper)相关的操作
		...
		//如果不能从数据后台获取upstrem代理后台，则尝试读取文件
        if (status != NGX_OK) {
            ngx_log_error(NGX_LOG_ERR, cycle->log, 0, 
		...
		//调用ngx_http_upsync_packet_init_pt类型的init函数
...
//指令upstream_show的回调函数，指定，当接受http请求，显示当前upstream后台
static char *
